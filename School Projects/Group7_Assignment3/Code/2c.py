...
correct_answer = 0.4514116667901403133978501889534646984347655748004997

c = "0.4514116667901403133978501889534646984347655748004997"


######### help functions ########

def c_eval_y_points(X):
    Y = list()
    for i in range( len(X) ):
        if (X[i] != 0.0):
            y = math.sin(2.0*pi*X[i])/(2.0*pi*X[i])
        else:
            y = 1.0
        Y.append(y)
    return Y
...
print "\nTrapezoid Rule:"
print "The correct answer: "+ c 
for i in range(2574, 2700):
    X = c_divide_x(i, a, b)  
    Y = c_eval_y_points(X)
    result = c_trap(X, Y)
    print "Estimate Result: " + repr(result)
    if ( abs(result-correct_answer) < 0.0000001 ):
        print "Success at n = " + repr(i)
        print "Error = " + str(abs(result-correct_answer))
        break
